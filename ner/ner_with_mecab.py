...
def tag_data(data, words_for_tagging = None, target_index=2):
    ...
    for target,regex__, url, recipe_id, cate in tqdm(np.array(regex_val)):
        ...
        # for entity_candidate in text.split():
        for temp in text.split():
            # if entity_candidate not in ['',] and [i for i in m.pos(entity_candidate.strip())][-1][-1] in ['NNG','NNP','NNB','NNBC','NR','NP']:
            if temp not in ['',]:
...
def get_tagged_data(path, file_name, words_for_tagging = None):
    ...
    #                         f"INSERT INTO words_for_tagging (ner, pos, cate) values ('{j.strip()}','custom','ingr');##{target} #### {url}")
